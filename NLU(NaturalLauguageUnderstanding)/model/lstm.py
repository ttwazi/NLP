...
from keras.layers import Embedding, LSTM, Bidirectional,  Dense, Dropout, TimeDistributed
from .nn import NN
...
class biLSTM(NN):
    ...
    def _build(self):
        model = Sequential()
        model.add(Embedding(self.config['vocab_size'], self.config['embedding_dim'], input_length = self.config['maxlen'],embeddings_initializer = 'uniform', trainable = True, mask_zero = True))
    ...
    def fit_and_validate(self, train_x, train_y, sample_weight, validate_x, validate_y):
        self.model.fit(train_x, train_y, epochs = self.config['epochs'], \
                       verbose = True, validation_data = (validate_x, validate_y), \
    ...
    def evaluate(self, predictions, validate_x, validate_y):
        ...
        cls_report = classification_report(validate_y_masked, predictions_masked)
